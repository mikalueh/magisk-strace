...
print_modname() {
  ui_print " "
  ui_print "    ********************************************"
  ui_print "    *          Magisk-strace by mikalueh       *"
  ui_print "    ********************************************"
  ui_print " "
}
...
on_install() {
  ...
  $UNZIP -qq -o "$ZIPFILE" "files/strace" -j -d "$F_TARGETDIR"
  mv "$F_TARGETDIR/strace" "$F_TARGETDIR/strace"
}
...
set_permissions() {
  ...
  # Custom permissions
  set_perm $MODPATH/system/bin/strace 0 2000 0755 u:object_r:system_file:s0
}
